...
app.post('/api/notifications/subscribe', async (req, res) => {
  ...
  if (!updated) {
    return res.status(404).json({ error: 'User not found — userId may be invalid' });
  }

  if (!updated.push_subscription) {
    return res.status(500).json({ error: 'Subscription column exists but value was not saved — check RLS policies on users table allow UPDATE for service role' });
  }
